...
def heuristic_send(state):
    ...
    for planet in state.my_planets() + state.not_my_planets():
        ...
        heuristic_value, closest_planet_that_can_attack = evaluate_planet (sorted_my_all, sorted_enemy_all, planet, state, init_owner)
...
def evaluate_planet (sorted_my_all, sorted_enemy_all, planet, state, init_owner):
    ...
    planet_owner, this_planet_ships = simulate_fleets_planet(sorted_my_curr, sorted_enemy_curr, planet, init_owner)
    ...
    return heuristic_value, closest_planet_that_can_attack
    
# returns planet owner, planet ships after simulation
def simulate_fleets_planet(sorted_my_curr, sorted_enemy_curr, planet, init_owner):
    my_iter = 0
    enemy_iter = 0
    turn_counter = 0
    ...
    while my_iter < len(sorted_my_curr) and enemy_iter < len(sorted_enemy_curr): # loop both lists ============================================
        ...
        if sorted_my_curr[my_iter].turns_remaining < sorted_enemy_curr[enemy_iter].turns_remaining:
            ...
            this_planet_ships += planet_owner * turns_since * planet.growth_rate # evals 0 for neutral
            
            if (planet_owner == 0):
                this_planet_ships -= sorted_my_curr[my_iter].num_ships
                if this_planet_ships < 0:
                    planet_owner = 1
                    this_planet_ships = abs(this_planet_ships)
            else:
                this_planet_ships += sorted_my_curr[my_iter].num_ships
                
            if this_planet_ships < 0:
                planet_owner = abs(planet_owner) * -1
